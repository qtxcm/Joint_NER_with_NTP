...
class Data:
    def __init__(self):
        ...
        self.type_alphabet = Alphabet('type', True, keep_growing=False)#type
        ...
        self.train_bert_dir = None
        self.dev_bert_dir = None
        self.test_bert_dir = None
        self.train_bert = None
        self.dev_bert = None
        self.test_bert  = None
        ...
        ## Training
        self.average_batch_loss = True #False
    ...
    def initial_feature_alphabets(self):
        ...
        if total_column > 1:
            for idx in range(1, total_column-5):
    ...
    def build_alphabet_type(self, input_file):
        in_lines = open(input_file,'r').readlines()
        dict_type = dict()
        for line in in_lines:
            if len(line) > 2:
                ...
                if("-" in label):
                    label_type = label.split("-")[1].strip().upper()
                    label_pos = label.split("-")[0].strip().upper()
                    if(label_pos == "B"):
                        if dict_type.__contains__(label_type):
                            dict_type[label_type] += 1
                        else:
                            dict_type[label_type] = 1

        tup = sorted(dict_type.items(), key=lambda d: d[1], reverse=True)
        for item in tup:
            self.type_alphabet.add(item[0])
            print(item[0],"  ",item[1])

        self.type_alphabet_size = self.type_alphabet.size()
        #self.type_size = len(tup)
    ...
    def generate_instance(self, name):
        self.fix_alphabet()
        if name == "train":
            self.train_texts, self.train_Ids = read_instance(self.train_dir, self.word_alphabet, self.char_alphabet, self.feature_alphabets, self.label_alphabet, self.type_alphabet, self.number_normalized, self.MAX_SENTENCE_LENGTH,self.train_bert)
        elif name == "dev":
            self.dev_texts, self.dev_Ids = read_instance(self.dev_dir, self.word_alphabet, self.char_alphabet, self.feature_alphabets, self.label_alphabet, self.type_alphabet,self.number_normalized, self.MAX_SENTENCE_LENGTH,self.dev_bert)
        elif name == "test":
            self.test_texts, self.test_Ids = read_instance(self.test_dir, self.word_alphabet, self.char_alphabet, self.feature_alphabets, self.label_alphabet, self.type_alphabet,self.number_normalized, self.MAX_SENTENCE_LENGTH,self.test_bert)
        elif name == "raw":
            self.raw_texts, self.raw_Ids = read_instance(self.raw_dir, self.word_alphabet, self.char_alphabet, self.feature_alphabets, self.label_alphabet, self.type_alphabet,self.number_normalized, self.MAX_SENTENCE_LENGTH)
        else:
    ...
    def load_bert_embs(self, data_file):
        f = open(data_file, 'rb')
        tmp_dict = pickle.load(f)
        f.close()
        return tmp_dict
    ...
    def read_config(self,args):
        ## read data:
        config = config_file_to_dict(args)
        ## read data:
        the_item = 'train_dir'
        ...
        the_item = 'train_bert_dir'
        if the_item in config:
            self.train_bert_dir = config[the_item]
        the_item = 'dev_bert_dir'
        if the_item in config:
            self.dev_bert_dir = config[the_item]
        the_item = 'test_bert_dir'
        if the_item in config:
            self.test_bert_dir = config[the_item]
        ...
        the_item = 'device'
        if the_item in config:
            self.device = torch.device(config[the_item])
        the_item = 'clip_grad'
        if the_item in config:
            self.clip_grad = float(config[the_item])
        the_item = 'label_embedding_scale'
        if the_item in config:
            self.label_embedding_scale = float(config[the_item])
        the_item = 'num_attention_head'
        if the_item in config:
            self.num_attention_head = int(config[the_item])
        the_item = 'whether_clip_grad'
        if the_item in config:
            self.whether_clip_grad = str2bool(config[the_item])
